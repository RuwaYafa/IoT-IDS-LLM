...
class Prompter:
    def __init__(self, tokenizer):
        ...
        templates = {
            ...
            "Qwen": {
                "instruction_template":
                    ...
                    "<|im_start|>user\n{user_message}<|im_end|>\n"
                    "<|im_start|>assistant\n",
                "response_template": "{assistant_response}<|im_end|>"
            },
           
            # "meta-llama": {"instruction_template": "[INST] <<SYS>>\n{system_message}\n<</SYS>>\n\n{user_message} [/INST]",
            #                 "response_template": "{assistant_response} </s>"},
        ...
        self.instruction_template = templates.get(self.model_family, templates["meta-llama"])["instruction_template"]
        self.response_template = templates.get(self.model_family, templates["meta-llama"])["response_template"]
    ...
    def _for_sft(self, messages):
        ...
        if self.tokenizer.chat_template is None:
            ...
            # {% for message in messages %}
            # {% if message['role'] == 'system' %}<s>'[INST]' {{ message['content'] + '\n'}}
            ...
            #     {% set instruction = '[INST]' + message['content'] + eos_token + '\n' %}
            # {% elif message['role'] == 'user' %}
            #     {% set instruction = instruction + message['content'] + eos_token + '[/INST]' %}
            # {% elif message['role'] == 'assistant' %}
            #     {% set response = message['content'] + eos_token %}
            # {% endif %}
            # {% endfor %}
            # """
